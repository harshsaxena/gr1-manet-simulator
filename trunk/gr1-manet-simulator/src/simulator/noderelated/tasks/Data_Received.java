...
public class Data_Received extends Thread {
	...
	public Data_Received(String name, Node mynode, DataPacket packet,
			Node receivedFrom) {
		super(name);
		this.mynode = mynode;
		this.packet = packet;
	...
	public void run() {
		if (packet.dest.equals(mynode)) {
			// ConsoleLogger.logger.info("Node"+
			// mynode+":received DataPacket from "+
			...
			if (!Route.isBad(route)) {
				...
				// " : passing DataPacket from " + packet.source +
				// " which handed from " + receivedFrom + " to " +
				// route.getNext_hop());
				FileLogger.write("Node " + mynode
						+ ": passing DataPacket from " + packet.source
						+ " which handed from " + receivedFrom + " to "
						+ route.getNext_hop(), FileLogger.MSG_TYPE_INFO);

				route.setLifeTime(new Date().getTime()
						+ Node.ACTIVE_ROUTE_TIMEOUT);
				Route route2 = mynode.search(packet.source);
				if (route2 != null) {
					route2.setLifeTime(new Date().getTime()
							+ Node.ACTIVE_ROUTE_TIMEOUT);
				}
				...
				// ConsoleLogger.logger.debug("Node"+
				// mynode+":receiving DataPacket from "+
				// packet.source+" which handed from "+receivedFrom+"but have no route!");
